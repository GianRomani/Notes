...
But the types and nature for the attacks are several and always evolving (just think about the spreading of multimodal models, the variety of kinds of inputs also causes a wider range of possible threats). [[Reasoning LLMs]] introduce additional attack surfaces through chain-of-thought hijacking and unfaithful reasoning — see [[Security of Reasoning Models]] for a dedicated analysis. For more details check [[Vulnerabilities in LLM-base applications]].
## Prompt attacks
...
With the rise of [[AI Agents]] and agentic systems, the attack surface has expanded significantly to include tool use, multi-agent communication, and autonomous decision-making. For security considerations specific to agentic systems, see [[AI Agent Security]]. Beyond prompt injection, the landscape includes threats such as [[Model Theft]] and [[Training Data Poisoning]], which represent distinct attack vectors against model integrity and data security.