  ...
  if (graph_is_local) {
    nodes = new vis.DataSet(
      graph_data.nodes.filter(
        (node) => node.id == curr_node.id || connected_nodes.includes(node.id),
      ),
    );
  } else {
...
if (
  ...
  graph.interactionHandler.body &&
  graph.interactionHandler.body.eventListeners &&
  graph.interactionHandler.body.eventListeners.onMouseWheel
) {
  var originalOnMouseWheel =
    graph.interactionHandler.body.eventListeners.onMouseWheel;
